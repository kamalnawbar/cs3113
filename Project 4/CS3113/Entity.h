...
enum AIType       { WANDERER, FOLLOWER, PATROL          };
...
private:
    ...
    float mPatrolStart = 0.0f;
    float mPatrolEnd   = 0.0f;
...
public:
    ...
    bool isCollidingWith(Entity *other){ return isColliding(other);           }
    ...
    void setPatrolRange(float start, float end)
        { mPatrolStart = start; mPatrolEnd = end;  }
};
