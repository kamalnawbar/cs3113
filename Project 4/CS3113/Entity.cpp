...
{
    if (mIsCollidingLeft || mPosition.x <= mPatrolStart)
        moveRight();
    else if (mIsCollidingRight || mPosition.x >= mPatrolEnd)
        moveLeft();
...
void Entity::render()
{
    ...
    case ATLAS:
        ...
        // flip horizontalement si on va a gauche
        if (mFlipX) textureArea.width = -textureArea.width;
